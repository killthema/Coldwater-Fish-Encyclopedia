...
public class FishController {
    ...
    private final FishRepository fishRepository; // DB 담당 (창고지기)
    private final FishService fishService;       // AI 담당 (요리사)
...
// [수정] 리액트가 보내는 'name'이라는 글자를 'keyword' 변수에 담으라고 강제 지정합니다.
    ...
    public ResponseEntity<?> predictFish(@RequestParam("file") MultipartFile file) {
        ...
        if (fish.isPresent()) {
            // Case A: DB에 정보가 있으면 -> 그 정보를 그대로 줌 (BEST!)
            return ResponseEntity.ok(fish.get());
        } else {
            // Case B: AI는 맞췄는데, DB에 상세 설명이 없을 때 -> 안내 메시지를 만들어서 줌
